import { InternalChatnelsEventsType } from './enums';
export const htmlTemplate = (orgDomain, serviceProvider, sessionToken, initView) => '<!DOCTYPE html><html lang="en"><head>' + '<meta charset="utf-8" />' + '<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, minimum-scale=1, user-scalable=no, viewport-fit=cover" />' + '<title>Chatnels</title>' + `<script>
    (function(w, d){
      ...
      a.onerror = function(message) {
        w.ReactNativeWebView.postMessage(JSON.stringify({type: "${InternalChatnelsEventsType.LOAD_SCRIPT_ERROR}", data: message}));
      };
