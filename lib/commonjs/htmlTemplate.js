...
const htmlTemplate = (orgDomain, serviceProvider, sessionToken, initView) => '<!DOCTYPE html><html lang="en"><head>' + '<meta charset="utf-8" />' + '<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, minimum-scale=1, user-scalable=no, viewport-fit=cover" />' + '<title>Chatnels</title>' + `<script async src="https://statics.${serviceProvider}/admin/js/chatnels.client.js"></script>` + `<script>
    window.chatnelsAsync = function() {
      if(window.ChatnelsClient) {
        ...
        window.ChatnelsClient.on('chatnels:message', function(data) {
          console.log('chatnels:message :', data);
