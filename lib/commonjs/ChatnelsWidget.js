...
const ChatnelsWidget = /*#__PURE__*/React.forwardRef((_ref, ref) => {
  ...
  const handleMessage = React.useCallback(event => {
    try {
      ...
      if (type === _enums.CHANTELS_EVENT_TYPE.LOAD_SCRIPT_ERROR) {
        handleError(new Error(data));
      } else if (type === 'reAuth') {
        if (onRequestSession) {
          onRequestSession();
        }
      } else if (type === _enums.CHANTELS_EVENT_TYPE.APP_READY) {
      ...
      } else if (type === _enums.CHANTELS_EVENT_TYPE.APP_REQUEST_FOCUS) {
  ...
  const handleShouldStartLoadWithRequest = React.useCallback(request => {
    if (request.url === 'chatnels://local.chatnels.com/' || request.url === 'about:blank') {
      ...
      if (onChatnelsEvent) {
        onChatnelsEvent(_enums.CHANTELS_EVENT_TYPE.EXTERNAL_URL, {
          url: request.url
        });
      }
