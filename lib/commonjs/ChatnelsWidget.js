...
const ChatnelsWidget = /*#__PURE__*/React.forwardRef((_ref, ref) => {
  ...
  const handleMessage = React.useCallback(event => {
    try {
      ...
      if (type === _enums.ChatnelsEventsType.LOAD_SCRIPT_ERROR) {
        handleError(new Error(data));
      } else if (type === 'reAuth') {
        if (onRequestSession) {
          onRequestSession();
        }
      } else if (type === _enums.ChatnelsEventsType.APP_READY) {
      ...
      } else if (type === _enums.ChatnelsEventsType.APP_REQUEST_FOCUS) {
